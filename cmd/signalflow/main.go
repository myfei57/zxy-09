...
import (
	...
	"signalflow/internal/console"
	"signalflow/internal/engine"
)

func main() {
	addr := flag.String("addr", "", "listen address (overrides SIGNALFLOW_LISTEN)")
	dataDir := flag.String("data", "", "data directory (overrides SIGNALFLOW_DATA)")
	...
	log.Printf("SignalFlow listening on %s, data dir %s", cfg.ListenAddr, cfg.DataDir)
